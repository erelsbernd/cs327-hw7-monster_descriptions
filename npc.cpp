...
void npc_next_pos_line_of_sight(dungeon_t *d, character *c, pair_t next)
{
  ...
  dir[dim_y] = character_get_y(d->thepc) - c->position[dim_y];
  dir[dim_x] = character_get_x(d->thepc) - c->position[dim_x];
  if (dir[dim_y]) {
...
static void npc_next_pos_00(dungeon_t *d, npc *c, pair_t next)
{
  ...
  if (can_see(d, character_get_pos(c), character_get_pos(d->thepc), 0)) {
    c->pc_last_known_position[dim_y] = character_get_y(d->thepc);
    c->pc_last_known_position[dim_x] = character_get_x(d->thepc);
